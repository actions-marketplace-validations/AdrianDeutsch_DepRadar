...
- **Self-contained single-file, NOT Native AOT.** The hand-rolled mediator registers
  ...
  that does not matter for a CI tool. Compressed single-file lands at ~38 MB and works
  unmodified — the binary alone, in an empty directory, runs a full registry scan
  (verified).
...
- ~190 MB of release assets per version (5 × ~38 MB) — irrelevant on GitHub releases.
